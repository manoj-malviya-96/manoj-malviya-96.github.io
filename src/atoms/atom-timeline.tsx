...
import {Timeline} from "primereact/timeline"; // Assuming you have a Card component
...
const AtomTimeline: React.FC<TimelineProps> = ({items}) => {
    ...
    const makeCard = (item: TimelineItemProps) => {
        return (
            <AtomCard image={item.image} title={item.title} onClick={item.onClick}/>
        );
    };

    return (
        <Timeline align="alternate" className="customized-timeline"
                  value={items} content={makeCard} marker={makeMarker}/>
    )
}
