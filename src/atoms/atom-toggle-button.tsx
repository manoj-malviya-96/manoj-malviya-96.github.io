...
const AtomToggleButton: React.FC<ToggleButtonProps> = React.memo(
  ({
    ...
    if (!offLabel && !onLabel && !offIcon && !onIcon) {
      throw new Error(
        "At least one of the following props must be set: offLabel, onLabel, offIcon, onIcon",
      );
    }
    ...
    return (
      <AtomButton
        ...
        severity={checked ? ButtonSeverity.Secondary : ButtonSeverity.Primary}
        onClick={onToggle}
        type={checked ? ButtonType.Solid : props.type}
      />
    );
  },
);
