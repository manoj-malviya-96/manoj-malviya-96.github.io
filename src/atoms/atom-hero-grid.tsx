...
const AtomHeroGridItem: React.FC<AtomHeroGridItemProps> = React.memo(
  ({ icon, title, summary, link, className }) => {
    const hasSub = title || icon;
    return (
      <div
        className={`flex flex-col gap-4 rounded-lg p-8 w-full h-full
  ...
  ({ contentList, className = "" }) => {
    return (
      <AtomBentoBox
        ...
        columns={2}
        component={
          AtomHeroGridItem as unknown as React.ComponentType<BentoBoxItemProps>
        }
      />
    );
  },
