...
function isOklchAndConvert(input: string): any {
  ...
  const oklchRegex =
    /^oklch\((\d+(\.\d+)?%)\s+(\d+(\.\d+)?)\s+(\d+(\.\d+)?)(\s*\/\s*\d+(\.\d+)?)?\)$/i;
  ...
  try {
    ...
    throw new Error(
      `Failed to convert OKLCH to RGBA: ${(error as Error).message}`,
    );
  }
}
...
export function getScaleColor(
  ...
  const result: Array<string> = [lastColor];
  ...
  for (let i = 1; i <= numStops; i++) {
    ...
    result.push(adjustColor(brandColor, 1, intensity));
  }
