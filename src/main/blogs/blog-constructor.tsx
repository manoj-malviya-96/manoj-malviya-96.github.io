  ...
  ({ name, title, children }) => {
    return (
      <Element
        name={name}
        className={
          "flex flex-col gap-4 w-full h-fit items-center my-16 mx-auto"
        }
      >
        <AtomTitleText className={"text-left w-full md:w-3/4 md:mr-48"}>
          {title}
        </AtomTitleText>
        <div
          className={"w-full md:w-3/4 md:mr-48 justify-center items-center "}
        >
          {children}
        </div>
      </Element>
          ...
          {tabs.length > 1 &&
            breakpoint !== ScreenSizeBreakPointAsString.Small && (
              <AtomTableOfContents
                sections={tabs}
                label={"Contents"}
                className={"sticky top-32 self-start w-1/5 my-16 z-5"}
              />
            )}
